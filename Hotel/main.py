...
def registrar_habitacion():
    ...
    while True:
        ...
        if any(habitacion[1] == numero_habitacion for habitacion in habitaciones_existentes):
...
def registrar_pasajero():
    ...
    try:
        ...
        pasajeros_a_registrar -= 1  # Descontar el pasajero responsable
        ...
        print(f"{pasajeros_a_registrar + 1} pasajeros registrados en la habitación {id_habitacion_responsable}.")
        ...
        if pasajeros_restantes == 0:
            print("Todos los pasajeros han sido registrados exitosamente.")
        else:
            print("Quedaron pasajeros sin registrar.")

        # Preguntar si desea hacer más registros o volver al menú
        opcion = input("\n¿Desea registrar más pasajeros? (s/n): ").lower()
        if opcion == 's':
            registrar_pasajero()
    ...
    try:
        ...
        if habitaciones:
            while True:
                ...
                for habitacion in habitaciones:
                    id_habitacion, numero_habitacion, pasajeros_admitidos, orientacion, estado = habitacion
                    table.rows.append([id_habitacion, numero_habitacion, pasajeros_admitidos, orientacion, estado])
                ...
                while True:
                    try:
                        ...
                        habitaciones_existentes = habitacion_dao.obtener_habitaciones()
                        if any(habitacion[1] == nuevo_numero and habitacion[0] != id_habitacion for habitacion in habitaciones_existentes):
